...
export class FirebaseTransLoader implements TranslateLoader {
  ...
  public getTranslation(lang: string) {
    switch (lang) {
      case 'en':
        return this.store.collection('translations').doc('english').valueChanges();
      case 'fi':
        return this.store.collection('translations').doc('finnish').valueChanges();
      case 'ru':
        return this.store.collection('translations').doc('russian').valueChanges();
      default:
        return this.store.collection('translations').doc('english').valueChanges();
    }
  }
